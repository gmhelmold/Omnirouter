...
def test_generator_rls_creates_files():
    """Generator must run successfully and create files."""
    from generators.database.rls import generate_rls

    output_dir = tempfile.mkdtemp()
    try:
        result = generate_rls(output_dir)
...
def test_generator_rls_idempotent():
    """Running twice must be safe — second run succeeds or is no-op."""
    from generators.database.rls import generate_rls

    output_dir = tempfile.mkdtemp()
    try:
        result1 = generate_rls(output_dir)
        result2 = generate_rls(output_dir)
        assert isinstance(result2, dict)
    finally:
        shutil.rmtree(output_dir)


def test_generator_rls_valid_python():
...
def test_generator_rls_files_exist():
