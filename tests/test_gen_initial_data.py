"""Tests for generator: initial_data."""
...
def test_generator_initial_data_idempotent():
    """Running twice must be safe — second run succeeds or is no-op."""
    from generators.infra.initial_data import generate_initial_data

    output_dir = tempfile.mkdtemp()
    try:
        result1 = generate_initial_data(output_dir)
        result2 = generate_initial_data(output_dir)
        assert isinstance(result2, dict)
    finally:
        shutil.rmtree(output_dir)


def test_generator_initial_data_valid_python():
...
def test_generator_initial_data_files_exist():
