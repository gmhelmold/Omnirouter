...
def test_generator_schemas_idempotent():
    """Running twice must be safe — second run succeeds or is no-op."""
    from generators.auth.schemas import generate_auth_schemas

    output_dir = tempfile.mkdtemp()
    try:
        result1 = generate_auth_schemas(output_dir)
        result2 = generate_auth_schemas(output_dir)
        assert isinstance(result2, dict)
    finally:
        shutil.rmtree(output_dir)


def test_generator_schemas_valid_python():
...
def test_generator_schemas_files_exist():
