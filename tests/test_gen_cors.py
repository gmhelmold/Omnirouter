...
def test_generator_cors_idempotent():
    """Running twice must be safe — second run succeeds or is no-op."""
    from generators.middleware.cors import generate_cors

    output_dir = tempfile.mkdtemp()
    try:
        result1 = generate_cors(output_dir, origins=None)
        result2 = generate_cors(output_dir, origins=None)
        assert isinstance(result2, dict)
    finally:
        shutil.rmtree(output_dir)


def test_generator_cors_valid_python():
...
def test_generator_cors_files_exist():
