...
def test_generator_health_idempotent():
    """Running twice must be safe — second run succeeds or is no-op."""
    from generators.endpoints.health import generate_health_checks

    output_dir = tempfile.mkdtemp()
    try:
        result1 = generate_health_checks(output_dir, check_db=True, check_redis=False)
        result2 = generate_health_checks(output_dir, check_db=True, check_redis=False)
        assert isinstance(result2, dict)
    finally:
        shutil.rmtree(output_dir)


def test_generator_health_valid_python():
...
def test_generator_health_files_exist():
