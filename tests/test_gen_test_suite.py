"""Tests for generator: test_suite."""
...
def test_generator_test_suite_creates_files():
    """Generator must run successfully and create files."""
    from generators.testing.test_suite import generate_test_suite

    output_dir = tempfile.mkdtemp()
    try:
        result = generate_test_suite(output_dir, models={"Item": {"name": "str"}}, owner_models={}, with_auth=True, shared_models={})
...
def test_generator_test_suite_idempotent():
    """Running twice must be safe — second run succeeds or is no-op."""
    from generators.testing.test_suite import generate_test_suite

    output_dir = tempfile.mkdtemp()
    try:
        result1 = generate_test_suite(output_dir, models={"Item": {"name": "str"}}, owner_models={}, with_auth=True, shared_models={})
        result2 = generate_test_suite(output_dir, models={"Item": {"name": "str"}}, owner_models={}, with_auth=True, shared_models={})
        assert isinstance(result2, dict)
    finally:
        shutil.rmtree(output_dir)


def test_generator_test_suite_valid_python():
...
def test_generator_test_suite_files_exist():
