"""Tests for generator: list_response."""
...
def test_generator_list_response_idempotent():
    """Running twice must be safe — second run succeeds or is no-op."""
    from generators.schemas.list_response import generate_list_response

    output_dir = tempfile.mkdtemp()
    try:
        result1 = generate_list_response(output_dir, name="Item")
        result2 = generate_list_response(output_dir, name="Item")
        assert isinstance(result2, dict)
    finally:
        shutil.rmtree(output_dir)


def test_generator_list_response_valid_python():
...
def test_generator_list_response_files_exist():
