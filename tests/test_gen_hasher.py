...
def test_generator_hasher_idempotent():
    """Running twice must be safe — second run succeeds or is no-op."""
    from generators.auth.hasher import generate_password_hasher

    output_dir = tempfile.mkdtemp()
    try:
        result1 = generate_password_hasher(output_dir)
        result2 = generate_password_hasher(output_dir)
        assert isinstance(result2, dict)
    finally:
        shutil.rmtree(output_dir)


def test_generator_hasher_valid_python():
...
def test_generator_hasher_files_exist():
