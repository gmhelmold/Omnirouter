...
def test_generator_conftest_creates_files():
    ...
    try:
        result = generate_test_infrastructure(output_dir, with_auth=True)
...
def test_generator_conftest_idempotent():
    """Running twice must be safe — second run succeeds or is no-op."""
    from generators.testing.conftest import generate_test_infrastructure

    output_dir = tempfile.mkdtemp()
    try:
        result1 = generate_test_infrastructure(output_dir, with_auth=True)
        result2 = generate_test_infrastructure(output_dir, with_auth=True)
        assert isinstance(result2, dict)
    finally:
        shutil.rmtree(output_dir)


def test_generator_conftest_valid_python():
...
def test_generator_conftest_files_exist():
