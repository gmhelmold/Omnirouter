"""Tests for generator: user_routes."""
...
def test_generator_user_routes_idempotent():
    """Running twice must be safe — second run succeeds or is no-op."""
    from generators.endpoints.user_routes import generate_user_routes

    output_dir = tempfile.mkdtemp()
    try:
        result1 = generate_user_routes(output_dir, with_superuser_management=True)
        result2 = generate_user_routes(output_dir, with_superuser_management=True)
        assert isinstance(result2, dict)
    finally:
        shutil.rmtree(output_dir)


def test_generator_user_routes_valid_python():
...
def test_generator_user_routes_files_exist():
