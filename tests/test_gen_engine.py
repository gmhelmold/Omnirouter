...
def test_generator_engine_creates_files():
    """Generator must run successfully and create files."""
    from generators.database.engine import generate_engine

    output_dir = tempfile.mkdtemp()
    try:
        result = generate_engine(output_dir, driver='asyncpg', pool_size=5, max_overflow=10)
...
def test_generator_engine_idempotent():
    """Running twice must be safe — second run succeeds or is no-op."""
    from generators.database.engine import generate_engine

    output_dir = tempfile.mkdtemp()
    try:
        result1 = generate_engine(output_dir, driver='asyncpg', pool_size=5, max_overflow=10)
        result2 = generate_engine(output_dir, driver='asyncpg', pool_size=5, max_overflow=10)
        assert isinstance(result2, dict)
    finally:
        shutil.rmtree(output_dir)


def test_generator_engine_valid_python():
...
def test_generator_engine_files_exist():
