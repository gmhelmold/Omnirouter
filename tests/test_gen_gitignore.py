...
def test_generator_gitignore_idempotent():
    """Running twice must be safe — second run succeeds or is no-op."""
    from generators.infra.gitignore import generate_gitignore

    output_dir = tempfile.mkdtemp()
    try:
        result1 = generate_gitignore(output_dir)
        result2 = generate_gitignore(output_dir)
        assert isinstance(result2, dict)
    finally:
        shutil.rmtree(output_dir)


def test_generator_gitignore_valid_python():
...
def test_generator_gitignore_files_exist():
