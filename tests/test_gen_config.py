...
def test_generator_config_creates_files():
    """Generator must run successfully and create files."""
    from generators.infra.config import generate_config

    output_dir = tempfile.mkdtemp()
    try:
        result = generate_config(output_dir, with_db=True, with_redis=False, with_smtp=True, with_sentry=False, prefix="/api/v1")
...
def test_generator_config_idempotent():
    """Running twice must be safe — second run succeeds or is no-op."""
    from generators.infra.config import generate_config

    output_dir = tempfile.mkdtemp()
    try:
        result1 = generate_config(output_dir, with_db=True, with_redis=False, with_smtp=True, with_sentry=False, prefix="/api/v1")
        result2 = generate_config(output_dir, with_db=True, with_redis=False, with_smtp=True, with_sentry=False, prefix="/api/v1")
        assert isinstance(result2, dict)
    finally:
        shutil.rmtree(output_dir)


def test_generator_config_valid_python():
...
def test_generator_config_files_exist():
