"""Tests for generator: input_schema."""
...
def test_generator_input_schema_creates_files():
    """Generator must run successfully and create files."""
    from generators.schemas.input_schema import generate_input_schema

    output_dir = tempfile.mkdtemp()
    try:
        result = generate_input_schema(output_dir, name="Item", fields={"name": "str"}, validators=None)
...
def test_generator_input_schema_idempotent():
    """Running twice must be safe — second run succeeds or is no-op."""
    from generators.schemas.input_schema import generate_input_schema

    output_dir = tempfile.mkdtemp()
    try:
        result1 = generate_input_schema(output_dir, name="Item", fields={"name": "str"}, validators=None)
        result2 = generate_input_schema(output_dir, name="Item", fields={"name": "str"}, validators=None)
        assert isinstance(result2, dict)
    finally:
        shutil.rmtree(output_dir)


def test_generator_input_schema_valid_python():
...
def test_generator_input_schema_files_exist():
