...
def test_generator_routes_idempotent():
    """Running twice must be safe — second run succeeds or is no-op."""
    from generators.auth.routes import generate_auth_routes

    output_dir = tempfile.mkdtemp()
    try:
        result1 = generate_auth_routes(output_dir, with_password_recovery=True)
        result2 = generate_auth_routes(output_dir, with_password_recovery=True)
        assert isinstance(result2, dict)
    finally:
        shutil.rmtree(output_dir)


def test_generator_routes_valid_python():
...
def test_generator_routes_files_exist():
