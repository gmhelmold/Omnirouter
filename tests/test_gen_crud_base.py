...
def test_generator_crud_base_idempotent():
    """Running twice must be safe — second run succeeds or is no-op."""
    from generators.database.crud_base import generate_crud_base

    output_dir = tempfile.mkdtemp()
    try:
        result1 = generate_crud_base(output_dir)
        result2 = generate_crud_base(output_dir)
        assert isinstance(result2, dict)
    finally:
        shutil.rmtree(output_dir)


def test_generator_crud_base_valid_python():
...
def test_generator_crud_base_files_exist():
