"""Tests for generator: rate_limit."""
...
def test_generator_rate_limit_idempotent():
    """Running twice must be safe — second run succeeds or is no-op."""
    from generators.auth.rate_limit import generate_rate_limit

    output_dir = tempfile.mkdtemp()
    try:
        result1 = generate_rate_limit(output_dir)
        result2 = generate_rate_limit(output_dir)
        assert isinstance(result2, dict)
    finally:
        shutil.rmtree(output_dir)


def test_generator_rate_limit_valid_python():
...
def test_generator_rate_limit_files_exist():
