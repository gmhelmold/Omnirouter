...
def test_generator_app_creates_files():
    """Generator must run successfully and create files."""
    from generators.infra.app import generate_app

    output_dir = tempfile.mkdtemp()
    try:
        result = generate_app(output_dir, name="Item", prefix="/api/v1", with_sentry=False, with_rate_limit=True, with_prometheus=True)
...
def test_generator_app_idempotent():
    """Running twice must be safe — second run succeeds or is no-op."""
    from generators.infra.app import generate_app

    output_dir = tempfile.mkdtemp()
    try:
        result1 = generate_app(output_dir, name="Item", prefix="/api/v1", with_sentry=False, with_rate_limit=True, with_prometheus=True)
        result2 = generate_app(output_dir, name="Item", prefix="/api/v1", with_sentry=False, with_rate_limit=True, with_prometheus=True)
        assert isinstance(result2, dict)
    finally:
        shutil.rmtree(output_dir)


def test_generator_app_valid_python():
...
def test_generator_app_files_exist():
