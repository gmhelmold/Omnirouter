...
def test_generator_alembic_idempotent():
    """Running twice must be safe — second run succeeds or is no-op."""
    from generators.database.alembic import generate_alembic

    output_dir = tempfile.mkdtemp()
    try:
        result1 = generate_alembic(output_dir)
        result2 = generate_alembic(output_dir)
        assert isinstance(result2, dict)
    finally:
        shutil.rmtree(output_dir)


def test_generator_alembic_valid_python():
...
def test_generator_alembic_files_exist():
