"""Shared framework for gap-fill BEHAVIOR scenarios (split from
``test_behavior_scenarios_gap_fill.py`` to respect the 500-LOC cap).

This module carries NO collectable tests — only the scenario framework,
patch templates, fixture helpers, and the ``_run_scenario`` driver that
each ``__partN`` file imports.
...
# ---------------------------------------------------------------------------
# pytest integration — shared parametrized test runner
# ---------------------------------------------------------------------------
