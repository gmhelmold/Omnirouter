"""Tests for generator: security_headers."""
...
def test_generator_security_headers_creates_files():
    ...
    try:
        result = generate_security_headers(output_dir)
...
def test_generator_security_headers_idempotent():
    """Running twice must be safe — second run succeeds or is no-op."""
    from generators.middleware.security_headers import generate_security_headers

    output_dir = tempfile.mkdtemp()
    try:
        result1 = generate_security_headers(output_dir)
        result2 = generate_security_headers(output_dir)
        assert isinstance(result2, dict)
    finally:
        shutil.rmtree(output_dir)


def test_generator_security_headers_valid_python():
...
def test_generator_security_headers_files_exist():
