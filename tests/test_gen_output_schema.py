"""Tests for generator: output_schema."""
...
def test_generator_output_schema_creates_files():
    """Generator must run successfully and create files."""
    from generators.schemas.output_schema import generate_output_schema

    output_dir = tempfile.mkdtemp()
    try:
        result = generate_output_schema(output_dir, name="Item", fields={"name": "str"}, exclude_fields=None)
...
def test_generator_output_schema_idempotent():
    """Running twice must be safe — second run succeeds or is no-op."""
    from generators.schemas.output_schema import generate_output_schema

    output_dir = tempfile.mkdtemp()
    try:
        result1 = generate_output_schema(output_dir, name="Item", fields={"name": "str"}, exclude_fields=None)
        result2 = generate_output_schema(output_dir, name="Item", fields={"name": "str"}, exclude_fields=None)
        assert isinstance(result2, dict)
    finally:
        shutil.rmtree(output_dir)


def test_generator_output_schema_valid_python():
...
def test_generator_output_schema_files_exist():
