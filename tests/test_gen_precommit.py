...
def test_generator_precommit_idempotent():
    """Running twice must be safe — second run succeeds or is no-op."""
    from generators.infra.precommit import generate_precommit

    output_dir = tempfile.mkdtemp()
    try:
        result1 = generate_precommit(output_dir, python_version='3.12')
        result2 = generate_precommit(output_dir, python_version='3.12')
        assert isinstance(result2, dict)
    finally:
        shutil.rmtree(output_dir)


def test_generator_precommit_valid_python():
...
def test_generator_precommit_files_exist():
