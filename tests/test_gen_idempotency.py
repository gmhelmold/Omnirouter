"""Tests for generator: idempotency."""
...
def test_generator_idempotency_creates_files():
    ...
    try:
        result = generate_idempotency_middleware(output_dir, ttl_seconds='60 * 60 * 24')
...
def test_generator_idempotency_idempotent():
    """Running twice must be safe — second run succeeds or is no-op."""
    from generators.middleware.idempotency import generate_idempotency_middleware

    output_dir = tempfile.mkdtemp()
    try:
        result1 = generate_idempotency_middleware(output_dir, ttl_seconds='60 * 60 * 24')
        result2 = generate_idempotency_middleware(output_dir, ttl_seconds='60 * 60 * 24')
        assert isinstance(result2, dict)
    finally:
        shutil.rmtree(output_dir)


def test_generator_idempotency_valid_python():
...
def test_generator_idempotency_files_exist():
