"""Tests for generator: github_actions."""
...
def test_generator_github_actions_creates_files():
    ...
    try:
        result = generate_github_actions(output_dir, python_version='3.12', postgres_version='16')
...
def test_generator_github_actions_idempotent():
    """Running twice must be safe — second run succeeds or is no-op."""
    from generators.deployment.github_actions import generate_github_actions

    output_dir = tempfile.mkdtemp()
    try:
        result1 = generate_github_actions(output_dir, python_version='3.12', postgres_version='16')
        result2 = generate_github_actions(output_dir, python_version='3.12', postgres_version='16')
        assert isinstance(result2, dict)
    finally:
        shutil.rmtree(output_dir)


def test_generator_github_actions_valid_python():
...
def test_generator_github_actions_files_exist():
