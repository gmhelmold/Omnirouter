...
def test_generator_readme_idempotent():
    """Running twice must be safe — second run succeeds or is no-op."""
    from generators.infra.readme import generate_readme

    output_dir = tempfile.mkdtemp()
    try:
        result1 = generate_readme(output_dir, name="Item", prefix="/api/v1")
        result2 = generate_readme(output_dir, name="Item", prefix="/api/v1")
        assert isinstance(result2, dict)
    finally:
        shutil.rmtree(output_dir)


def test_generator_readme_valid_python():
...
def test_generator_readme_files_exist():
