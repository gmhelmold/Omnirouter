"""Tests for generator: correlation."""
...
def test_generator_correlation_creates_files():
    ...
    try:
        result = generate_correlation_id(output_dir)
...
def test_generator_correlation_idempotent():
    """Running twice must be safe — second run succeeds or is no-op."""
    from generators.middleware.correlation import generate_correlation_id

    output_dir = tempfile.mkdtemp()
    try:
        result1 = generate_correlation_id(output_dir)
        result2 = generate_correlation_id(output_dir)
        assert isinstance(result2, dict)
    finally:
        shutil.rmtree(output_dir)


def test_generator_correlation_valid_python():
...
def test_generator_correlation_files_exist():
