...
def test_generator_errors_idempotent():
    """Running twice must be safe — second run succeeds or is no-op."""
    from generators.endpoints.errors import generate_error_handlers

    output_dir = tempfile.mkdtemp()
    try:
        result1 = generate_error_handlers(output_dir)
        result2 = generate_error_handlers(output_dir)
        assert isinstance(result2, dict)
    finally:
        shutil.rmtree(output_dir)


def test_generator_errors_valid_python():
...
def test_generator_errors_files_exist():
