"""Tests for generator: dockerfile."""
...
def test_generator_dockerfile_creates_files():
    """Generator must run successfully and create files."""
    from generators.infra.dockerfile import generate_dockerfile

    output_dir = tempfile.mkdtemp()
    try:
        result = generate_dockerfile(output_dir, python_version='3.12', port=8000, workers=4)
...
def test_generator_dockerfile_idempotent():
    """Running twice must be safe — second run succeeds or is no-op."""
    from generators.infra.dockerfile import generate_dockerfile

    output_dir = tempfile.mkdtemp()
    try:
        result1 = generate_dockerfile(output_dir, python_version='3.12', port=8000, workers=4)
        result2 = generate_dockerfile(output_dir, python_version='3.12', port=8000, workers=4)
        assert isinstance(result2, dict)
    finally:
        shutil.rmtree(output_dir)


def test_generator_dockerfile_valid_python():
...
def test_generator_dockerfile_files_exist():
