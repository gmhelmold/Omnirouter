"""Tests for generator: dockerfile_lint."""
...
def test_generator_dockerfile_lint_creates_files():
    """Generator must run successfully and create files."""
    from generators.deployment.dockerfile_lint import analyze_dockerfile

    output_dir = tempfile.mkdtemp()
    try:
        result = analyze_dockerfile(output_dir)
...
def test_generator_dockerfile_lint_idempotent():
    """Running twice must be safe — second run succeeds or is no-op."""
    from generators.deployment.dockerfile_lint import analyze_dockerfile

    output_dir = tempfile.mkdtemp()
    try:
        result1 = analyze_dockerfile(output_dir)
        result2 = analyze_dockerfile(output_dir)
        assert isinstance(result2, dict)
    finally:
        shutil.rmtree(output_dir)


def test_generator_dockerfile_lint_valid_python():
...
def test_generator_dockerfile_lint_files_exist():
