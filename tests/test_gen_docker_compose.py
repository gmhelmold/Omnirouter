"""Tests for generator: docker_compose."""
...
def test_generator_docker_compose_creates_files():
    ...
    try:
        result = generate_docker_compose(output_dir, services=None, postgres_version='16', redis=False, app_port=8000)
...
def test_generator_docker_compose_idempotent():
    """Running twice must be safe — second run succeeds or is no-op."""
    from generators.deployment.docker_compose import generate_docker_compose

    output_dir = tempfile.mkdtemp()
    try:
        result1 = generate_docker_compose(output_dir, services=None, postgres_version='16', redis=False, app_port=8000)
        result2 = generate_docker_compose(output_dir, services=None, postgres_version='16', redis=False, app_port=8000)
        assert isinstance(result2, dict)
    finally:
        shutil.rmtree(output_dir)


def test_generator_docker_compose_valid_python():
...
def test_generator_docker_compose_files_exist():
