"""Tests for generator: encryption."""
...
def test_generator_encryption_idempotent():
    """Running twice must be safe — second run succeeds or is no-op."""
    from generators.database.encryption import generate_encryption

    output_dir = tempfile.mkdtemp()
    try:
        result1 = generate_encryption(output_dir)
        result2 = generate_encryption(output_dir)
        assert isinstance(result2, dict)
    finally:
        shutil.rmtree(output_dir)


def test_generator_encryption_valid_python():
...
def test_generator_encryption_files_exist():
