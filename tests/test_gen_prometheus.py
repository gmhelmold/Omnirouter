"""Tests for generator: prometheus."""
...
def test_generator_prometheus_creates_files():
    ...
    try:
        result = generate_prometheus_metrics(output_dir, prefix="/api/v1")
...
def test_generator_prometheus_idempotent():
    """Running twice must be safe — second run succeeds or is no-op."""
    from generators.observability.prometheus import generate_prometheus_metrics

    output_dir = tempfile.mkdtemp()
    try:
        result1 = generate_prometheus_metrics(output_dir, prefix="/api/v1")
        result2 = generate_prometheus_metrics(output_dir, prefix="/api/v1")
        assert isinstance(result2, dict)
    finally:
        shutil.rmtree(output_dir)


def test_generator_prometheus_valid_python():
...
def test_generator_prometheus_files_exist():
